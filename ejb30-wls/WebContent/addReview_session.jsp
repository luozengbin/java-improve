...
<%
  ...
  List<Book>     books = serviceImpl.getAll(Book.class);
  if (books == null || books.isEmpty()) {
    out.println("Found no book in the database");
    return;
  }

  List<Reviewer> reviewers = serviceImpl.getAll(Reviewer.class);
  ...
  List<Review> reviewsOfBook = serviceImpl.getReviewsByItem(book.getId());
  List<Review> reviewsByReviewer = serviceImpl.getReviewsByReviewer(reviewer.getName());
%>
<HR>
...
<%
  serviceImpl.newReview(reviewer, book, rating, comment);

  List<Review> newReviewsOfBook = serviceImpl.getReviewsByItem(book.getId());
  List<Review> newReviewsByReviewer = serviceImpl.getReviewsByReviewer(reviewer.getName());
%>
